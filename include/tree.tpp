...
void tree<T>::copy(const node &src, node &dest) {
  ...
  if (!src.left_child().is_null()){
    ...
    if (!dest.left_child().is_null()){
      ...
      node curr_node = dest.left_child().right_sibling();
      while (!curr_node.is_null()){
        curr_node.set_parent(dest);
...
tree<T>::begin_preorder() {
  return tree::preorder_iterator(this->root);
}

template<typename T>
typename tree<T>::preorder_iterator tree<T>::end_preorder() {
  return tree::preorder_iterator(node());
}
...
tree<T>::cbegin_preorder() const {
  return tree::const_preorder_iterator(this->root);
}

template<typename T>
typename tree<T>::const_preorder_iterator
tree<T>::cend_preorder() const {
  return tree::const_preorder_iterator(node());
}
